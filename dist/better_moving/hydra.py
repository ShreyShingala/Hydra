...
focused_window = None  # Tracks the focused window
stop_event = threading.Event()
max_windows = 10  # Maximum number of windows
...
def hydrafunc():
    ...
    def spawner():
        global windows
        if len(windows) < max_windows:  # Check the window limit
            create_window()
        else:
            print("Window cap reached.")

    def create_window():
        global root, remove_entry, result_label, focused_window, windows
        ...
        x.protocol("WM_DELETE_WINDOW", lambda: windows.remove(x) if x in windows else None)  # Remove from list on close
        ...
        def move_windows_dvd():
            dx, dy = 2, 2
            screen_width = x.winfo_screenwidth()
            screen_height = x.winfo_screenheight()
            win_width = 600
            win_height = 225
            x_pos, y_pos = rm.randint(0, screen_width - win_width), rm.randint(0, screen_height - win_height)
            ...
            while not stop_event.is_set():
                if x != focused_window:
                    ...
                    if x_pos <= 0 or x_pos + win_width >= screen_width:
                        dx = -dx
                    if y_pos <= 0 or y_pos + win_height >= screen_height:
                        dy = -dy

                    x.geometry(f'{win_width}x{win_height}+{x_pos}+{y_pos}')

                time.sleep(0.02)

        threading.Thread(target=move_windows_dvd, daemon=True).start()
