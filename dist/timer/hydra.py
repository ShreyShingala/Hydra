...
def hydrafunc():
    ...
    def create_window():
        global root, remove_entry, result_label, timer_labels
        ...
        # Timer Label
        timer_label = ttk.Label(x, text="00:00", anchor="center", font=("DS-Digital", 24, "bold"), foreground="red")
        timer_label.pack(pady=(10, 10))
        timer_labels.append(timer_label)  # Add to global list
        ...
        x.title("April Fools")
        x.geometry('600x300-' + str(rm.randint(0, screen_width - 600)) + '+' + str(rm.randint(0, screen_height - 300)))
    ...
    def countdown():
        global timervalue, timer_labels
        if timervalue > 0:
            timervalue -= 1
            minutes, seconds = divmod(timervalue, 60)
            time_text = f"{minutes}:{seconds:02}"
            for label in timer_labels:  # Update all labels
                label.config(text=time_text)
            root.after(1000, countdown)  # Schedule the next tick
        else:
            spawner()  # Spawn new windows if timer ends

    # Start the first window and the countdown
    ...
    countdown()  # Start the synchronized countdown
